...
class ReportFetching extends Component {
  constructor() {
    ...
    this.state = {
      ...
      Time_Obs: "",
      bandera: true,
      CantonFilter: "Esto es una prueba",
    };
  }
  ...
  async getReports() {
    ...
    const response = await db
      .collection("report")
      .where("OBSERVACION", "==", cantonSeleccionado)
      .get();
  ...
  render() {
    const [cantonSeleccionado, setcantonSeleccionado] = useState("ninguno");
    ...
    return (
      <div>
        <Container fluid>
          ...
          <Select
            onChange={(opt) => setcantonSeleccionado(opt.value)}
